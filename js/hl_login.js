...
$(function () {
    ...
    var signIN = function () {
        ...
        var indexPath=domain.replace("/ReportServer","")+'/ReportServer?op=fs';
		//domain="/WebReport";
            ...
            complete: function (res, status) {
                if (res.responseText == "") {
                	...
                	if (signResult.fail) {
                		...
                		alert(signResult.msg);
                	} else {
                		if("不是超级管理员"==signResult.msg){
                			window.location.href="http://60.212.191.147:8082/ReportServer?formlet=navigation.frm";
                		}else{
                			window.location.href="http://60.212.191.147:8082/ReportServer?op=fs";
                		}
